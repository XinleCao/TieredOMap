...
def write_convergence_csv(path, results):
    with open(path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["queries_K", "B_obs", "hit_rate_pct"])
        for B_obs in sorted(results.keys()):
            for queries_k, hit_rate in results[B_obs]:
                writer.writerow([f"{queries_k:.1f}", B_obs, f"{hit_rate:.2f}"])


def write_drift_csv(path, results):
    with open(path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["onset_queries_K", "B_swap", "hit_rate_pct"])
        for label, data in results.items():
            if label.startswith("B_swap="):
                b_swap = label.split("=", 1)[1]
            else:
                b_swap = "no_maint"
            for onset_queries_k, hit_rate in data:
                writer.writerow([f"{onset_queries_k:.1f}", b_swap, f"{hit_rate:.2f}"])
...
def main():
    args = sys.argv[1:]
    smoke = "--smoke" in args
    args = [arg for arg in args if arg != "--smoke"]
    outdir = args[0] if args else "results_paper"
    ...
    if smoke:
        ...
        s = 1.0
        B = 4
        B_obs_list = [1024, 2048]
        B_swap_convergence = 16
        B_swap_drift_list = [4, 8]
        B_obs_drift = 2048
        total_queries = 20_000
        report_convergence = 2_000
        post_onset = 20_000
        report_drift = 1_000
        runs = [(14, "_smoke")]
    else:
    ...
    for logN, tag in runs:
        ...
        write_convergence_csv(os.path.join(outdir, f"dynamic{tag}.csv"), conv)
        ...
        write_drift_csv(os.path.join(outdir, f"drift{tag}.csv"), drift)
